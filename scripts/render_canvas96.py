...
Reads  assets/original/cipher_render/sheets/{idle,walk,run,dash,kneel,aim}_{facing}.png
...
# is checked exactly — a missing name would rewrite twenty-three strips and
# leave the twenty-fourth stale in sheets96, the partial-set lie the molder
...
EXPECTED = {f"{verb}_{facing}.png"
            for verb in ("idle", "walk", "run", "dash", "kneel", "aim")
...
def reframe(strip: Image.Image, name: str) -> tuple[Image.Image, list[int]]:
    ...
    n = w // REN_CANVAS
    standing = name.startswith(("idle_", "aim_"))
    ...
    for i in range(n):
        ...
        # law's line. The idle and aim strips are standing references (a
